...
const ScrollBox = <T extends { id: string }>(
  ...
  const { entries: watcherEntries } = useIntersectionObserverV2(watcherRef);
  ...
  return (
    <div className={`${styles.scrollBox} ${wrapperClassName}`}>
      <ul className={styles.list} ref={listRef}>
        ...
        {list.map((item, i) => (
          <li
            key={item.id}
            className={`${styles.item} ${currentIndex === i ? 'current' : ''}`}
            ref={r => {
      ...
      <button
        className={`${styles.navButton} ${styles.prev} ${
          buttonEnabled.prev ? styles.navButtonOn : ''
        }`}
        onClick={() => move('prev')}
      />
      <button
        className={`${styles.navButton} ${styles.next} ${
          buttonEnabled.next ? styles.navButtonOn : ''
        }`}
        onClick={() => move('next')}
      />
    </div>
